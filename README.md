# 👋 Sup, I'm Lucc

> Student learning to code · Currently breaking things in Python & Java · Send help.

![Python](https://img.shields.io/badge/Python-3572A5?style=flat&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-b07219?style=flat&logo=java&logoColor=white)

---

## 🚧 Projects / Ideas

| | |
|---|---|
| 💡 | **Untitled project #1** — Exists only in my head... Very polished up there though. |
| 🧠 | **Learning Python & Java** —  Normal dev stuff. |
| 🔭 | **Future projects TBD** — Will update this when I figure out what I'm doing with my life. |

## ⚡ Facts

- 🐍 I know a bit of Python and Java (emphasis on *bit*)
- 🌱 Currently: learning, breaking things, Googling the error, repeat
- 💬 Ask me about: absolutely nothing yet, I just got here
---

## 📫 How to reach me

> DISCORD!1!!!! -- > lucc2
[![Discord](https://img.shields.io/badge/Discord-5865F2?style=flat&logo=discord&logoColor=white)](https://discord.com/users/794065111005790208)
---

idk what im doing rw with my life

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Luis-2011&color=blue&style=flat" alt="profile views"/>
</p>
